...
def main():
    parser = argparse.ArgumentParser(
        description="Compute Diarization Error Rate (DER) using pyannote's metrics."
    )
    parser.add_argument(
        "--segments",
        type=str,
        required=True,
        help="Path to the segments file (ground truth).",
    )
    parser.add_argument(
        "--rttm",
        type=str,
        required=True,
        help="Glob pattern to match RTTM files (e.g. '*.rttm').",
    )
    args = parser.parse_args()

    # Load ground-truth annotations.
    reference = load_reference(args.segments)
    if not reference:
        print("No valid reference annotations were loaded.")
        return

    # Load system output (RTTM) annotations.
    hypothesis = load_hypotheses(args.rttm)
    if not hypothesis:
